...
class SupabaseClient:
    ...
    def fetch_reference_table(self, table_name: str) -> List[Dict]:
        """Fetch all records from a reference table"""
        try:
    ...
    def insert_smartject(self, smartject_data: Dict) -> bool:
        """Insert a new smartject into the database"""
        try:
            ...
            if "duplicate key" in str(e).lower() or "already exists" in str(e).lower():
                logger.error("This appears to be a duplicate entry")
            return False

    def insert_smartject_industries(self, relations: List[Dict]) -> bool:
        """Insert smartject-industry relations"""
        try:
            if relations:
                self.client.table("smartject_industries").insert(relations).execute()
            return True
        except Exception as e:
            logger.error(f"Error inserting industry relations: {e}")
            return False

    def insert_smartject_audience(self, relations: List[Dict]) -> bool:
        """Insert smartject-audience relations"""
        try:
            if relations:
                self.client.table("smartject_audience").insert(relations).execute()
            return True
        except Exception as e:
            logger.error(f"Error inserting audience relations: {e}")
            return False

    def insert_smartject_functions(self, relations: List[Dict]) -> bool:
        """Insert smartject-business function relations"""
        try:
            if relations:
                self.client.table("smartject_business_functions").insert(relations).execute()
            return True
        except Exception as e:
            logger.error(f"Error inserting function relations: {e}")
            return False
        ...
        try:
            while True:
                response = self.client.table("smartjects") \
                    .select("id, title, team, image_url") \
